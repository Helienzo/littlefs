...
static int dir_compact_done(lfs_t *lfs, int retval);

static int dir_compact_register_callback(lfs_t *lfs, lfs_ssize_t (*cb)(struct lfs *lfs, lfs_ssize_t retval)) {
    if (cb != NULL) {
        lfs->workspace.dir_compact.dir_compact_done_cb = cb;
        return LFS_ERR_OK;
    } else {
        lfs->workspace.dir_compact.dir_compact_done_cb = NULL;
        return LFS_ERR_INVAL;
    }
}
static int dir_compact_commit_start(lfs_t *lfs) {
    // setup commit state
    lfs->workspace.dir_compact.commit.block = lfs->workspace.dir_compact.dir->pair[1];
    lfs->workspace.dir_compact.commit.off = 0;
    lfs->workspace.dir_compact.commit.ptag = 0xffffffff;
    lfs->workspace.dir_compact.commit.crc = 0xffffffff;
    lfs->workspace.dir_compact.commit.begin = 0;
    lfs->workspace.dir_compact.commit.end = (lfs->cfg->metadata_max ?
                    lfs->cfg->metadata_max : lfs->cfg->block_size) - 8;

    // erase block
    if (lfs->action_complete_cb == NULL || lfs->workspace.dir_compact.dir_compact_done_cb == NULL) {
        // This is a blocking call
        // Make sure to clear callback
        lfs->lfs_bd_callbacks.erase_cb = NULL;
        // Call Erase
        int err = lfs_bd_erase(lfs, lfs->workspace.dir_compact.dir->pair[1]);
        // Call next state
        return dir_compact_erase_done(lfs->cfg, err);
    } else {
        // Set callback
        lfs->lfs_bd_callbacks.erase_cb = dir_compact_erase_done;
        // Call Erase
        return lfs_bd_erase(lfs, lfs->workspace.dir_compact.dir->pair[1]);
    }
}
...
static int dir_compact_relocate(lfs_t *lfs) {
    ...
    lfs->workspace.dir_compact.relocated = true;

    lfs_cache_drop(lfs, &lfs->pcache);
    if (!lfs->workspace.dir_compact.tired) {
        LFS_DEBUG("Bad block at 0x%"PRIx32, lfs->workspace.dir_compact.dir->pair[1]);
    }

    // can't relocate superblock, filesystem is now frozen
    if (lfs_pair_cmp(lfs->workspace.dir_compact.dir->pair, (const lfs_block_t[2]){0, 1}) == 0) {
        LFS_WARN("Superblock 0x%"PRIx32" has become unwritable",
                lfs->workspace.dir_compact.dir->pair[1]);
        return dir_compact_done(lfs, LFS_ERR_NOSPC);
    }

    // relocate half of pair
    int err = lfs_alloc(lfs, &lfs->workspace.dir_compact.dir->pair[1]);
    if (err && (err != LFS_ERR_NOSPC || !lfs->workspace.dir_compact.tired)) {
        return dir_compact_done(lfs, err);
    }

    lfs->workspace.dir_compact.tired = false;
    // Restart loop to commit compaction to blocks until a compact sticks
    return dir_compact_commit_start(lfs);
}

static int dir_compact_done(lfs_t *lfs, int retval) {
    if (retval == 0) {
        // If the command was successful, update retval
        retval = lfs->workspace.dir_compact.relocated ? LFS_OK_RELOCATED : 0;
    }

    if (lfs->workspace.dir_compact.dir_compact_done_cb != NULL) {
        // Call caller if this was a non blocking call
        return lfs->workspace.dir_compact.dir_compact_done_cb(lfs, retval);
    }
    else {
        return retval;
    }
...
static int lfs_dir_compact(lfs_t *lfs,
    ...
    lfs->workspace.dir_compact.relocated = false;
    lfs->workspace.dir_compact.tired = lfs_dir_needsrelocation(lfs, dir);

    // Store workspace context
    lfs->workspace.dir_compact.dir = dir;
    lfs->workspace.dir_compact.attrs = attrs;
    lfs->workspace.dir_compact.attrcount = attrcount;
    lfs->workspace.dir_compact.source = source;
    lfs->workspace.dir_compact.begin = begin;
    lfs->workspace.dir_compact.end = end;
    ...
    if (lfs->workspace.dir_compact.tired && lfs_pair_cmp(dir->pair, (const lfs_block_t[2]){0, 1}) != 0) {
...
static int ctz_extend_alloc(lfs_t *lfs) {
        ...
        // call erase
        return ctz_extend_block_erase(lfs);
}
...
static lfs_ssize_t file_flush_register_callback(lfs_t *lfs, lfs_ssize_t (*cb)(struct lfs *lfs, lfs_ssize_t retval)) {
    if (cb != NULL) {
        lfs->workspace.file_flush.file_flush_done_cb = cb;
        return LFS_ERR_OK;
    }
    else {
        lfs->workspace.file_flush.file_flush_done_cb = NULL;
        return LFS_ERR_INVAL;
    }
}

static lfs_ssize_t file_flush_flushedwrite_done_cb(lfs_t *lfs, lfs_ssize_t retval) {
    if (retval < 0) {
    ...
    if (lfs->rcache.block != LFS_BLOCK_NULL) {
        lfs_cache_drop(lfs, &lfs->workspace.file_flush.orig.cache);
        lfs_cache_drop(lfs, &lfs->rcache);
    }
    // We might have to run copy again
    return file_flush_copy(lfs);
}

static int file_flush_copy(lfs_t *lfs) {
    if (lfs->workspace.file->pos < lfs->workspace.file->ctz.size) {
        // copy over a byte at a time, leave it up to caching
        // to make this efficient
        lfs_ssize_t res = lfs_file_flushedread(lfs, &lfs->workspace.file_flush.orig, &lfs->workspace.file_flush.data, 1);
        if (res < 0) {
            return file_flush_done(lfs, res);
        }

        // register next state in file_flush
        flushedwrite_register_callback(lfs, file_flush_flushedwrite_done_cb);
        return lfs_file_flushedwrite(lfs, lfs->workspace.file, &lfs->workspace.file_flush.data, 1);
    }
    else {
        // Call next state
...
static int file_flush_relocate(lfs_t *lfs) {
    LFS_DEBUG("Bad block at 0x%"PRIx32, lfs->workspace.file->block);
    int err = lfs_file_relocate(lfs, lfs->workspace.file);
    if (err) {
        // Finding new block failed
...
static int file_flush_done(lfs_t *lfs, int retval) {
    if (retval == LFS_ERR_OK) {
        // actual file updates, if the command was successfull
        lfs->workspace.file->ctz.head = lfs->workspace.file->block;
        lfs->workspace.file->ctz.size = lfs->workspace.file->pos;
...
static int lfs_file_flush(lfs_t *lfs, lfs_file_t *file) {
    ...
    if (file->flags & LFS_F_WRITING) {
        lfs->workspace.file_flush.pos = file->pos;

        if (!(file->flags & LFS_F_INLINE)) {
            // copy over anything after current branch
            // Store context
            lfs->workspace.file_flush.orig.ctz.head = file->ctz.head;
            lfs->workspace.file_flush.orig.ctz.size = file->ctz.size;
            lfs->workspace.file_flush.orig.flags = LFS_O_RDONLY;
            lfs->workspace.file_flush.orig.pos = file->pos;
            lfs->workspace.file_flush.orig.cache = lfs->rcache;
...
static lfs_ssize_t flushedwrite_find_block(lfs_t *lfs) {
    ...
    if (!(lfs->workspace.file->flags & LFS_F_WRITING) ||
        ...
        if (!(lfs->workspace.file->flags & LFS_F_INLINE)) {
            if (!(lfs->workspace.file->flags & LFS_F_WRITING) && lfs->workspace.file->pos > 0) {
                // find out which block we're extending from
                int err = lfs_ctz_find(lfs, NULL, &lfs->workspace.file->cache,
                        lfs->workspace.file->ctz.head, lfs->workspace.file->ctz.size,
                ...
                lfs_cache_zero(lfs, &lfs->workspace.file->cache);
            }
...
static lfs_ssize_t flushedwrite_done(lfs_t *lfs, lfs_ssize_t retval) {
    if (lfs->workspace.flushedwrite.flushedwrite_done_cb != NULL) {
        if (retval) {
            // Return error
...
static lfs_ssize_t lfs_file_rawwrite(lfs_t *lfs, lfs_file_t *file,
    ...
    if (!(file->flags & LFS_F_WRITING) && file->pos > file->ctz.size) {
        // fill with zeros
        lfs->workspace.rawwrite.pos = file->pos;
        file->pos = file->ctz.size;
